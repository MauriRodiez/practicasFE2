...
function renderizarErrores(listado) {
  // listado equivale a decir "datos" de la lînea 72
...
function validarInformacion(usuario) {
  ...
  if (!isNaN(usuario.nombre) || usuario.nombre.length < 3) {
    errores.push(
      "El nombre de usario debe tener mas de 3 caracteres y no ser un número"
    );
  }
  ...
  if (usuario.hobbies.length > 4 || usuario.hobbies.length == 0) {
    errores.push(
      "Sólo es posible seleccionar entre 4 hobbies, y como mínimo u1"
    );
  }
...
// 2 - mostrar al final del formulario un caja con la misma estructura que la caja de errores,
//     pero con la tonalidad verde
...
function mostrarMensajeExito(listado) {
  if (listado == 0) {
    ...
    divTemplate.style =
      "background:rgba(0, 255, 0, 0.2);padding:.5em 1em;color: red;margin: .5em 0;";
